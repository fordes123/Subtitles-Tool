...
public class FunctionAPI {
	...
	public FunctionAPI(JFrame f, DataAPI data, ViewAPI api, ColorAPI color, Side SidePanel, MainPanel MPanel,
			Setting setting, About about, Help help, UpdateLog log,Informations st_MOd,Timeline Tl_Mod,Replace Re_Mod) {
		// TODO 自动生成的构造函数存根
		this.data = data;
		this.api = api;
		this.color = color;
		...
		SidePanel.jb_Open.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (data.file == null)
					...
					String filename1 = "." + data.file.getName().substring(0, data.file.getName().length() - suffix.length() - 1); // 获取纯文件名
					...
					data.subtitles = data.Read(data.file, setting.L3_j2.isSelected(), (String) setting.L3_j3.getSelectedItem());// 读取文件内容到主界面
		...
		SidePanel.jb_Delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int Re = JOptionPane.showOptionDialog(f, "删除源文件的同时将退出程序，是否继续？", "此操作不可逆",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
